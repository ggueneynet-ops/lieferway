...
export async function loadRedeemableRewards(opts: {
  ...
  const restaurant = await prisma.restaurant.findUnique({
    ...
    select: { wayPointsEnabled: true, wayPointsDisabledByAdmin: true, wayPointsBudgetCents: true, wayPointsBudgetSpentCents: true },
  });
...
export async function previewCheckoutReward(opts: {
  ...
  if (split.restaurantShareCents > 0) {
    const budget = restaurant.wayPointsBudgetCents;
    if (budget != null) {
      const remaining = budget - (restaurant.wayPointsBudgetSpentCents ?? 0);
      if (split.restaurantShareCents > remaining) {
        return { ok: false as const, error: "WayPoints-Budget des Restaurants ist ausgeschöpft." };
      }
    }
  }
  return {
...
export async function redeemRewardForOrder(opts: {
  ...
  await prisma.$transaction([
    ...
    ...(split.restaurantShareCents > 0
      ? [
          prisma.restaurant.update({
            where: { id: opts.restaurantId },
            data: { wayPointsBudgetSpentCents: { increment: split.restaurantShareCents } },
          }),
        ]
      : []),
  ]);
...
export async function reverseWayPointsForOrder(
  ...
  if (redeem && redeem.status === "APPLIED" && redeem.pointsSpent > 0) {
    ...
    if (reversed.created) {
      ...
      if (redeem.restaurantShareCents > 0) {
        await prisma.restaurant.update({
          where: { id: order.restaurantId },
          data: { wayPointsBudgetSpentCents: { decrement: redeem.restaurantShareCents } },
        });
      }
    }
  }
...
export async function adminAdjustWayPoints(opts: {
  userEmail: string;
  delta: number;
  reason: string;
}) {
  ...
  if (Math.abs(delta) > 1_000_000) throw new Error("DELTA_TOO_LARGE");
  ...
  const user = await prisma.user.findFirst({
    ...
    select: { id: true, email: true, wayPointsBalance: true },
  });
  if (!user) throw new Error("USER_NOT_FOUND");
  const key = `ADJUST:${user.id}:${Date.now()}:${delta}:${reason.slice(0, 24)}`;
  const result = await appendLedger({
    userId: user.id,
    delta,
    type: "ADJUST",
    uniqueKey: key,
    title: formatLedgerTitle({ type: "ADJUST", rewardTitle: reason }),
  });
  return {
    userId: user.id,
    email: user.email,
