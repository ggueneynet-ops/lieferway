    ...
    <div className={flash ? "lw-kitchen-flash -mx-3 rounded-xl px-3 py-1 md:-mx-5 md:px-5" : ""}>
      ...
      <section className="mb-5">
        <h2 className="mb-3 flex items-center gap-2 font-display text-[1.2rem] font-semibold tracking-tight text-[#111827]">
          {t.rpNewOrder}
          {incoming.length > 0 ? (
            <span className="rounded-full bg-[#922A49] px-2.5 py-0.5 text-[12px] font-bold text-white">
              {incoming.length}
            </span>
          ) : null}
        </h2>
        {incoming.length === 0 ? (
          <p className="rounded-2xl border border-[#F8BBD0] bg-white p-4 text-[14px] text-[#6B7280] shadow-[0_6px_18px_rgba(146,42,73,0.06)]">
            {t.noIncoming}
                ...
                {picking === o.id ? (
                  ...
                  <>
                    <button
                      ...
                      className="h-14 min-w-32 flex-1 touch-manipulation rounded-2xl bg-[#922A49] px-6 text-base font-semibold text-white hover:bg-[#7A2340] disabled:opacity-50"
                    >
                      {t.accept}
                    </button>
                    <button
            ...
            >
              ...
              {o.status === "PREPARING" && (
                <button
                  ...
                  className="h-14 w-full rounded-2xl bg-[#922A49] px-6 text-base font-semibold text-white hover:bg-[#7A2340] disabled:opacity-50"
                >
      ...
      <div className="mt-5 grid grid-cols-2 gap-2">
        <Link
          ...
          className="flex h-14 items-center justify-center rounded-2xl bg-[#922A49] px-3 text-sm font-semibold text-white hover:bg-[#7A2340]"
        >
          {t.navMenu}
        </Link>
        <Link
          href="/restaurant/finance"
          className="flex h-14 items-center justify-center rounded-2xl border border-[#F8BBD0] bg-white px-3 text-sm font-semibold text-[#7A2340]"
        >
          {t.rpFinance}
        </Link>
      </div>
...
function OrderCard({
  ...
  return (
    <article
      className={`rounded-2xl border bg-white p-4 ${highlight ? "lw-new-ticket shadow-[0_8px_24px_rgba(146,42,73,0.12)]" : "border-[#E5E7EB]"}`}
    >
      {kicker ? (
        <p className="mb-2 text-[11px] font-bold uppercase tracking-wide text-[#922A49]">{kicker}</p>
      ) : null}
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className={`text-sm font-medium ${pickup ? "text-[#7A2340]" : "text-text-secondary"}`}>
            {pickup
              ? pickupLabel
    ...
    <div className="w-full space-y-2">
      ...
      <div className="flex flex-wrap gap-2">
        {PREP_CHIPS.map((min) => (
          <button
            ...
            disabled={disabled}
            onClick={() => onPick(min)}
            className="h-12 min-w-[3.25rem] flex-1 touch-manipulation rounded-xl bg-[#922A49] px-2 text-sm font-semibold text-white hover:bg-[#7A2340] disabled:opacity-50"
          >
            {min}
          </button>
        ))}
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className="h-12 shrink-0 touch-manipulation rounded-xl bg-[#922A49] px-4 text-sm font-semibold text-white disabled:opacity-50"
        >
          {t.accept}
        </button>
      </div>
