...
/** Public chrome only — never link /admin or /courier here. */
export async function SiteFooter() {
  const { t } = await getCopy();
  return (
    <footer data-public-footer="partner-only" className="mt-auto border-t border-border bg-surface">
      <div className="mx-auto flex max-w-6xl flex-col gap-8 px-4 py-10 sm:flex-row sm:items-start sm:justify-between">
        ...
        <nav aria-label={t.partner} className="flex flex-col gap-2.5 text-sm text-text-secondary">
