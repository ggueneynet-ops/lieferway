...
- Restaurant panel: live kitchen board (SSE + poll), accept/reject, status. **Gong** on new (PLACED) tickets repeats until none are new (Annehmen or Ablehnen); first tap unlocks audio. **Drucken / Bon** on Heute and Bestellungen opens an 80mm kitchen ticket (`window.print()` / OS Bon-Drucker) with the **restaurant logo** at the top (seed `logoUrl` e.g. `/media/logos/anadolu-grill.png`, embedded as a data URI so the print popup still shows it). Initials if no logo — never the Lieferway ChatGPT header mark. **Bestellungen** date filter (Heute / Gestern / Datum, Angenommen + Abgelehnt). Restaurant marks orders ready for **its own delivery**. **Lieferung**: editable Lieferzeit (min–max minutes on marketplace cards), Mindestbestellwert, Liefergebühr, radius.
