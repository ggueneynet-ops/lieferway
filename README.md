...
- Restaurant panel: live kitchen board (SSE + poll), accept/reject, status. **Drucken / Bon** on Heute and Bestellungen opens an 80mm kitchen ticket (`window.print()` / OS Bon-Drucker). **Bestellungen** date filter (Heute / Gestern / Datum, Angenommen + Abgelehnt). Restaurant marks orders ready for **its own delivery**. **Lieferung**: editable Lieferzeit (min–max minutes on marketplace cards), Mindestbestellwert, Liefergebühr, radius.
