...
- Restaurant panel: live kitchen board (SSE + poll), accept/reject, status. **Gong** on new (PLACED) tickets repeats until none are new (Annehmen or Ablehnen); first tap unlocks audio. **Drucken / Bon** on Heute and Bestellungen opens an 80mm kitchen ticket (`window.print()` / OS Bon-Drucker). **Bestellungen** date filter (Heute / Gestern / Datum, Angenommen + Abgelehnt). Restaurant marks orders ready for **its own delivery**. **Lieferung**: editable Lieferzeit (min–max minutes on marketplace cards), Mindestbestellwert, Liefergebühr, radius.
...
Marketplace layout follows common Turkish food-app patterns (location bar, cuisine rail, dense list, sticky cart) with a Yemeksepeti-like **pink** `#E91E63`. Logo is the locked pink scooter + cloche: header uses the single-piece lockup PNG only; splash uses vertical master A. Name stays **Lieferway**.
