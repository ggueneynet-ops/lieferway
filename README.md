...
- Restaurant panel: live kitchen board (SSE + poll), accept/reject, status. **Gong** on new (PLACED) tickets repeats until none are new (Annehmen or Ablehnen); first tap unlocks audio. **Lieferbon drucken** on Heute and Bestellungen opens an 80mm kitchen/courier slip (`window.print()`) with the **restaurant logo** at the top — not a legal invoice. **Bestellungen** date filter (Heute / Gestern / Datum, Angenommen + Abgelehnt). Restaurant marks orders ready for **its own delivery**. **Lieferung**: editable Lieferzeit (min–max minutes on marketplace cards), Mindestbestellwert, Liefergebühr, radius. **Finanzen**: monthly **Provisionsrechnung** (draft PDF).
